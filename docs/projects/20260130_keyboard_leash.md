...
![Photo showing problematic misaligned keyboard halves](20260130_keyboard_leash/tidy_keyboard.JPG)
...
In the spirit of simplicity, I choose a friction wedge retention mechanism to secure the two keyboard halves. This means no fancy locking features or hardware. Instead I’m using the natural angle of the existing VIP3 tenting feature and the weight of the keyboard to create a self locking mechanism.

This locking mechanism strongly resists the two keyboard halves from moving closer or further apart from each other in the horizontal direction. However, the mechanism is less effective at resisting the two halves from moving forward or back relative to each because the tenting feature can slide more easily in the slot in that direction.
...
To keep the design simple and aesthetically clean, I choose to use a fixed separation distance of 100 mm. This distance is measured relative to the bottom of the tenting feature and is roughly what I have used for years.
...
![Photo showing four prototype coupons and two printed full-scale brackets](20260130_keyboard_leash/leashed_keyboard.JPG)
...
I've very happy with the results. The brackets have a minimal appearance when installed. Most importantly, they solve the problem by drastically minimizing keyboard drift throughout the day!
